...
<script>
	...
	export default {
		...
		methods: {
			...
			isLoad: () => {
				if (bIsAlipayMini) {
					window.location.replace('https://puser.zjzwfw.gov.cn/sso/alipay.do?action=ssoLogin&servicecode=zyyjsgcw&goto=https://mapi.zjzwfw.gov.cn/web/mgop/gov-open/zj/2001907982/reserved/index.html');
				} else {
					window.location.replace('https://puser.zjzwfw.gov.cn/sso/mobile.do?action=oauth&scope=1&servicecode=zyyjsgcw&goto=https://mapi.zjzwfw.gov.cn/web/mgop/gov-open/zj/2001907982/reserved/index.html');
				}
			},
			watchApp: () => {
